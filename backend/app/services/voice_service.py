...
class VoiceService:
    ...
    async def dispatch_voice_call(
        ...
        await audit_service.log_event(
            ...
            reason=f"Hinglish voice recovery call requested for case {case.id}",
        ...
        await audit_service.log_event(
            ...
            reason=f"Sarvam AI Voice Call dispatched in {voice_result['mode']} mode.",
            metadata_json={
                "mode": voice_result["mode"],
                "provider": voice_result["provider"],
                "language_code": voice_result.get("language_code", "hi-IN"),
                "speaker": voice_result.get("speaker", "anushka")
            }
        )

        # Log VOICE_CALL_COMPLETED
        await audit_service.log_event(
            db=db,
            case_id=case.id,
            ...
            actor_id="SARVAM_AI_VOICE_ENGINE",
            reason=f"Hinglish voice call interaction completed ({voice_result['mode']} mode). Customer informed of recovery options.",
